...
export function DetectiveApp() {
  ...
  const solvedCount = useMemo(() => ethicsCases.filter((caseItem) => {
    const caseAnswer = answers[caseItem.id];
    return caseAnswer?.action !== undefined && caseAnswer.finalSignal && caseAnswer.changeReason;
  }).length, [answers]);
    ...
    <main className="app-shell">
      <header className="site-header">
        ...
        <div className="header-actions">
          ...
          {screen !== "home" && <div className="badge-rack" aria-label={`해결 배지 ${solvedCount}개`}>
            {ethicsCases.map((caseItem) => {
              const caseAnswer = answers[caseItem.id];
              const earned = caseAnswer?.action !== undefined && caseAnswer.finalSignal && caseAnswer.changeReason;
              return <i key={caseItem.id} className={earned ? "earned" : ""}>{earned ? caseItem.badge : "?"}</i>;
            })}
          </div>}
      ...
      {screen === "setup" && (
        <section className="setup-page page-enter">
          <div className="setup-intro">
            <span className="page-label">탐정 등록 · 장비 지급</span>
            <h1>별명을 정하고<br />네 가지 추리 도구를 챙겨요.</h1>
          ...
          <button className="primary-button large" disabled={!nickname.trim()} onClick={() => { setCaseIndex(0); setRevealedClues(0); go("scene"); }}>첫 사건 수신하기 <span>→</span></button>
        </section>
      )}
      ...
      {screen === "scene" && (
        <CasePage item={item} caseIndex={caseIndex} stage="사건 이해">
          ...
          {revealedClues === item.clues.length ? <section className="first-judgement page-enter">
          ...
          </section> : <section className="judgement-lock"><span aria-hidden="true">⌕</span><div><b>첫 추리는 아직 잠겨 있어요</b><p>오른쪽 단서 봉투를 모두 열면 판단할 수 있습니다.</p></div><i>{revealedClues} / {item.clues.length}</i></section>}
      ...
      {screen === "think" && (
        <CasePage item={item} caseIndex={caseIndex} stage="근거 찾기">
          ...
          {answer.evidence !== undefined && answer.affected !== undefined && answer.value && (
            <div className="reason-summary page-enter" role="status"><span>나의 근거 문장</span><p>확인한 단서 <b>{item.evidenceOptions[answer.evidence].label}</b>에 따르면, <b>{item.people[answer.affected].name}</b>에게 미칠 영향을 생각해 <b>{answer.value}</b>의 가치를 지켜야 합니다.</p></div>
          )}
          <PageActions><button className="secondary-button" onClick={() => go("scene")}>← 첫 추리 보기</button><button className="primary-button large" disabled={answer.evidence === undefined || answer.affected === undefined || !answer.value} onClick={() => go("talk")}>양쪽 심문실 입장 <span>→</span></button></PageActions>
        </CasePage>
      )}
    ...
    <>
      <section className="home-hero page-enter">
        <div className="home-copy">
          ...
          <div className="mission-teaser" aria-label="탐정 임무 구성"><p><b>4</b><span>긴급 사건</span></p><p><b>12</b><span>봉인 단서</span></p><p><b>4</b><span>해결 배지</span></p></div>
          <div className="home-actions"><button className="primary-button large" onClick={onStart}>첫 사건 수신하기 <span>→</span></button><button className="text-button" onClick={onTeacher}>40분 수업 운영안</button></div>
      ...
      <section className="case-preview">
        <div className="preview-heading"><span>4개의 생활 속 사건</span><h2>단서를 모으고, 추리하고, 해결 배지를 획득해요.</h2></div>
      ...
      <section className="home-footer-cta"><div><span>교실에서 바로 사용할 수 있어요</span><h2>단서 수집 → 첫 추리 → 양쪽 심문 → 사건 해결</h2></div><button className="primary-button large" onClick={onStart}>탐정 임무 시작 <span>→</span></button></section>
    </>
  );
}
...
function CaseProgress({ caseIndex, screen }: { caseIndex: number; screen: Screen }) {
  const stages: { id: Screen; label: string }[] = [{ id: "scene", label: "현장" }, { id: "think", label: "추리" }, { id: "talk", label: "심문" }, { id: "resolve", label: "해결" }];
